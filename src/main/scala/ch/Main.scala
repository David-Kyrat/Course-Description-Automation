...
object Main {
	 ...
	 */
	private def parseGuiInput(args: Array[String]): (ParArray[String], ParArray[Int]) = {
		if (args.length <= 0) {
		...
		val courses: ParArray[String] = if (!tmp(0).isBlank) tmp(0).split(",").par.map(formatter) else ParArray.empty[String]
		val studyPlans: ParArray[Int] = if (!tmp(1).isBlank) tmp(1).split(",").par.map(formatter andThen checkIdOrAbbrev) else ParArray.empty[Int]
		(courses, studyPlans)
	}
